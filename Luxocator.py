...
class Luxocator(wx.Frame):
    ...
    # GUI init
    def __init__(self, classifierPath, maxImageSize = 768, verboseSearchSession = False, verboseClassifier = False):
        ...
        self._searchCtrl.Bind(wx.EVT_TEXT_ENTER, self._onSearchEntered)
        self._searchCtrl.Bind(wx.EVT_SEARCHCTRL_SEARCH_BTN, self._onSearchEntered)
        self._searchCtrl.Bind(wx.EVT_SEARCHCTRL_CANCEL_BTN, self._onSearchCanceled)
        ...
        controlsSizer.Add(self._searchCtrl, 0, wx.ALIGN_CENTER_VERTICAL | wx.RIGHT, border)
        controlsSizer.Add((0, 0), 1) # Spacer
        controlsSizer.Add(self._labelStaticText, 0, wx.ALIGN_CENTER_VERTICAL)
        controlsSizer.Add((0, 0), 1) # Spacer
        controlsSizer.Add(self._prevButton, 0, wx.ALIGN_CENTER_VERTICAL | wx.LEFT | wx.RIGHT, border)
        controlsSizer.Add(self._nextButton, 0, wx.ALIGN_CENTER_VERTICAL)

        # root sizer

        self._rootSizer = wx.BoxSizer(wx.VERTICAL)
        self._rootSizer.Add(self._staticBitmap, 0, wx.TOP | wx.LEFT | wx.RIGHT, border)
        self._rootSizer.Add(controlsSizer, 0, wx.EXPAND | wx.ALL, border)
...
def main():
    os.environ['REQUESTS_CA_BUNDLE'] = \
            PyInstallerUtils.pyInstallerResourcePath('cacert.pem')
    app = wx.App()
    luxocator = Luxocator(
            PyInstallerUtils.pyInstallerResourcePath('classifier.mat'),
